...
class VVelocityMachineMotion:
    ...
    async def home(self):
        home_rate = 200
        for _ in range(200):
            time, state_a = await self._motor_a.get_limit_state()
            x1, y1, z1 = self.queue_planner._get_position_tail()
            if not state_a:
                await self.queue_planner.goto_and_await([x1-1,y1,z1], home_rate)
            else:
                x1,y1,z1 = self.queue_planner._get_position_tail()
                break
        
        await self.queue_planner.goto_and_await([x1+250,y1,z1], home_rate)
        for _ in range(400):
            time, state_b = await self._motor_b.get_limit_state()
            x2,y2,z2 = self.queue_planner._get_position_tail()
            if not state_b:
                await self.queue_planner.goto_and_await([x2+1,y2,z2], home_rate)
            else:
                x2,y2,z2 = self.queue_planner._get_position_tail()
                dx = (550-(x2-x1))/2
                dy = dx / np.tan(np.deg2rad(25))
